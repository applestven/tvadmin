...
import { RefreshCw, Search, Eye, RotateCcw } from 'lucide-react'
...
export default function DownloadPage() {
  ...
  const { getTasks, createTask, retryTask } = useTvApi()
  const pageSize = 10
  ...
  const handleRetry = async (task: TvTask) => {
    try {
      await retryTask(task.url, task.quality)
      fetchTasks()
    } catch (error) {
